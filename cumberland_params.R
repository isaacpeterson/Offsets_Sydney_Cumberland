initialise_user_global_params <- function(){
  ...
  # Needs to be FALSE when using the cumberland data
  ...
  # these are generated by the initialise_cumberland_params script so leave as FALSE
  ...
  # Takes the raster files and partitions them into their condition class feature objects
  # only time this needs to be set to TRUE is when anything regardin the conditon model params changes.
  ...
  # can set these to true to check if it avoides crashes.
  global_params$overwrite_management_dynamics = FALSE
  global_params$overwrite_feature_dynamics = FALSE # Set to TRUE if change no of time steps
...
#                                                                                        feature_params$simulated_time_vec)$y)))
  ...
  if( management_intensity == 'high') {
    mgmt_index_to_use = 3
  } else {
    if( management_intensity == 'low') {
      mgmt_index_to_use = 2  
    } else {
      stop("\nERROR: management_intensity must be set to either 'high' or 'low' ")
    }
  }
  ...
  management_dynamics_set = collate_dynamics(full_dynamics_set, management_datasheets, set_index_to_use = mgmt_index_to_use)
...
initialise_user_output_params <- function(){
  ...
  # If this is left as an empty vector, (ie 'vector()' ) will put in the
  # results in the collated realizations folder, otherwise you specify the
  # path to where the results should be stored here
  output_params$output_folder = vector() 

  # Can be 'impacts' or 'outcomes'
  output_params$plot_type = 'impacts'

  # Use 'all' for all, or otherwise the number (eg 6) means the first 6 realiztaions.
  ...
  # Ouput colur the full parcel comprising the offet a single colour, rather
  # than just changing the colour ramp for the vegetation
  output_params$output_block_offsets = TRUE
  
  # number of plot sub windows
  output_params$nx = 2 #3 
  output_params$ny = 2 #6
  ...
  output_params$landscape_outcome_plot_lims_set = list(rep(list(c(0, 2e6))))

  # Set of nested lists by scenario and feature (which in this case is 5) to
  # give the y limits of the output plots
